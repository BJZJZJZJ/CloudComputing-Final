...
[최종 결과물]<br/>
<img src="./readme_src/htu4.png" width=600px>
...
만일 '3'에서 넘어온 분석결과에 유명인이라는 정보가 포함되어 있다면, 로컬에서 자체적으로 동작중이던 Flask 서버에 데이터를 전송하고 Flask는 Imdb 라는 사이트를 웹크롤링해 결과를 웹서버로 넘겨준다. 이후 서버는 받은 결과물을 잘 가공해 사용자에게 보기 좋게 돌려준다.

기존에는 EC2를 이용해 가상 인스턴스에 웹서버를 구축해 하려고 했으나, CORS 문제를 쉽게 해결하지 못 해 불가피하게 개인PC에서 하게 되었다.
<br/><br/>
...
<img src="./readme_src/htu4.png" width=300px>

5. 결과버튼 확인을 누르면 해당 이미지에 대한 표정 분석 결과가 나오고, 유명인이라면 그의 출연작 리스트도 위 쪽의 사진과 같이 출력되게 된다.

<br/><br/>

## F. 개발 결과물의 필요성 및 활용방안

이름 모르는 배우의 사진을 통해 해당 배우의 출연작들을 확인할 수 있게 된다. <br>
또, 감정분석 결과를 통해 사람의 심리를 어느정도 확인해볼 수 있을 것으로 생각된다.
